    ...
    def __init__(self, data_params_path, data_ROI_path=None, image_sizes=None,rotation=None, selected_features=None):
        ...
        for i, features in enumerate(self.list_features):
            if self.data_ROI_path is not None:
                ...
                labeled_nodes_original = get_list_ROI(raw_x, raw_y, self.ROI_images[i].T, image_size=self.image_sizes[i])
...
def get_labels(Mouse_reference,warped_src,i):

    warped_denormalized=inverse_data_normalisation(warped_src, Mouse_reference.center_list[i])
    raw_x, raw_y = np.asarray(warped_denormalized[:, 0], dtype=int), np.asarray(warped_denormalized[:, 1], dtype=int)
    labeled_nodes = get_list_ROI(raw_x, raw_y, Mouse_reference.ROI_images[i].T,
                                 image_size=Mouse_reference.image_sizes[i])
